...
# Frozen generative critic + label/outcome advantage debug run
# Based on train_sokoban.sh, with critic disabled and extra debug logs.

USE_GEN_CRITIC="algorithm.use_label_outcome_advantage=True critic.enable=False generative_critic.enable=True"
...
MKL_SERVICE_FORCE_INTEL=1 python train.py --config-name _3_frozen_lake system.CUDA_VISIBLE_DEVICES=\"0,5,6,7\" trainer.n_gpus_per_node=4 \
    ...
    generative_critic.model_path=/mnt/kangshijia/models/Qwen3-8B \
    ...
    generative_critic.max_new_tokens=128 \
